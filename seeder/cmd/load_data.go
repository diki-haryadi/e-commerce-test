...
func runLoadData(cmd *cobra.Command, args []string) error {
	...
	envPaths := []string{
		...
		filepath.Join(filepath.Dir(callerDir), "..", "db/fixtures/orders.yml"),
		filepath.Join(filepath.Dir(callerDir), "..", "db/fixtures/order_items.yml"),
		filepath.Join(filepath.Dir(callerDir), "..", "db/fixtures/shop.yml"),
