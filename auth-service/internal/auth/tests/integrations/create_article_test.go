...
import (
	...
	articleDto "github.com/diki-haryadi/go-micro-template/internal/auth/dto"
	articleFixture "github.com/diki-haryadi/go-micro-template/internal/auth/tests/fixtures"
...
func (suite *testSuite) TestSuccessCreateHttpArticle() {
	...
	request := httptest.NewRequest(http.MethodPost, "/api/v1/auth", strings.NewReader(articleJSON))
	...
	caDto := new(articleDto.SignUpRequestDto)
	if assert.NoError(suite.T(), json.Unmarshal(response.Body.Bytes(), caDto)) {
		assert.Equal(suite.T(), "John Snow", caDto.Username)
		assert.Equal(suite.T(), "King of the north", caDto.Password)
	}
